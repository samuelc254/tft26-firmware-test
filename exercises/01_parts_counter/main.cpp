...
//          sometimes passing extremely fast (pulses as short as ~2 ms).
...
//     - MIN_VALID_PULSE_MS (2 ms): Validates the rising edge, keeping the system 
...
namespace {
    ...
    constexpr uint32_t MIN_VALID_PULSE_MS = 2;
...
int main() {
    ...
    while (true) {
        ...
        uint32_t current_count = count.load(std::memory_order_relaxed);

        bool keep_display_on = (current_time - last_display_update) >= DISPLAY_UPDATE_INTERVAL_MS;
